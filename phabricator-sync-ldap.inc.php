<?php
define("LIBPHUTIL_ROOT", ROOT . "/../libphutil");
require_once LIBPHUTIL_ROOT  . "/src/__phutil_library_init__.php";
$mail_str = "";
function debug($str) {
	global $mail_str;
...
function newCronContentSource() {
	return PhabricatorContentSource::newForSource ( PhabricatorCronContentSource::SOURCECONST );
}
...
function getField($object, $field_key, $user) {
	...
	$fields = PhabricatorCustomField::getObjectFields ( $object, $role )->setViewer ( $user )->readFieldsFromStorage ( $object )->getFields ();
	
	return idx ( $fields, $field_key )->getProxy ()->getFieldValue ();
}
function modifyProjectMembers($project, $members_diff, $viewer) {
	...
	if (DEBUG && ! empty ( $members_diff ['+'] )) {
		...
		foreach ( $members_diff ['+'] as $memberphid ) {
			$user = id ( new PhabricatorPeopleQuery () )->setViewer ( $viewer )->withPHIDs ( array (
					$memberphid 
			) )->executeOne ();
	...
	if (! DRY_RUN) {
		...
		$editor = id ( new PhabricatorProjectTransactionEditor () )->setActor ( $viewer )->setContentSource ( newCronContentSource () )->setContinueOnNoEffect ( true )->setContinueOnMissingFields ( true )->applyTransactions ( $project, $xactions );
	}
}
...
function map_users($phab_accounts, $ld, $map_empty = true) {
	...
	return array (
			"by_dn" => $userdn_userphid_map,
			"by_phid" => $userphid_userdn_map 
	);
}
function map_projects($phab_projects, $phab) {
	...
	foreach ( $phab_projects as $project ) {
		...
		if ($ldap_dn) {
			$ldap_dn = "cn=" . $ldap_dn . ",ou=Groups,dc=bitgear,dc=rs";
	...
	return array (
			"by_dn" => $groupdn_projectphid_map,
			"by_phid" => $projectphid_groupdn_map 
	);
}
...
function update_phab_users($user_map, $ldap_users, $ld, $phab) {
	...
	if (DEBUG && ! empty ( $users_diff ['-'] )) {
		...
		foreach ( $users_diff ['-'] as $userphid => $userdn ) {
			$user = id ( new PhabricatorPeopleQuery () )->setViewer ( $phab_admin )->withPHIDs ( array (
					$userphid 
			) )->executeOne ();
			$username = $user->getUserName ();
...
function update_phab_projects($project_map, $ldap_groups, $ld, $phab, $ldap_projects, $phab_projects) {
	$phab_admin = $phab ["admin"];
	...
	$projects_edit_diff = array ();
	
	if (CREATE_PROJECTS) {
		$groupdn_projectphid_map = $project_map ["by_dn"];
		foreach ( $ldap_groups as $key => $value ) {
			if (strpos($key, '.project') !== false) { 
				if (!isset ( $groupdn_projectphid_map [$key] )) {
					array_push ( $projects_diff ['+'], $key );
				} else {
					$projectdn = str_replace("=groups", "=projects", $key);
					$projectdn = str_replace("cn=", "uid=", $projectdn);
					$project = $ldap_projects [$projectdn];
					$phab_project = $phab_projects[$groupdn_projectphid_map [$key]];
					if ($project["displayname"][0] != $phab_project->getName()) {
						array_push ( $projects_edit_diff, $key);
					}
				}
			}
		}
	...
	if (!DEBUG && ! empty ( $projects_diff ['+'] )) {
		...
		foreach ( $projects_diff ['+'] as $groupdn ) {
			$group = $ldap_groups [$groupdn];
			$projectdn = str_replace("=groups", "=projects", $groupdn);
			$projectdn = str_replace("cn=", "uid=", $projectdn);
			$project = $ldap_projects [$projectdn];
			assert ( $project !== null );
			$groupnames = $project [$ldap_group_name_attr];
	...
	if (! DRY_RUN) {
		...
		foreach ( $projects_diff ['+'] as $groupdn ) {
			...
			array_push($data["transactions"], array("type" => "name", "value" => $project["displayname"][0]));
			array_push($data["transactions"], array("type" => "description", "value" => $project["displayname"][0]));
			array_push($data["transactions"], array("type" => "view", "value" => "PHID-PLCY-obtxrrbinx444phuhjle"));
			array_push($data["transactions"], array("type" => "edit", "value" => "admin"));
			array_push($data["transactions"], array("type" => "join", "value" => "obj.project.members"));
			array_push($data["transactions"], array("type" => "custom.bitgear-rs:ldap:dn", "value" => $project["uid"][0]));
			
			$api_token = getenv('API_TOKET');
			
			$client = new ConduitClient(getenv('PHABRICATIOR_URI'));
			$client->setConduitToken($api_token);
			
			$result = $client->callMethodSynchronous('project.edit', $data);
		}
		
		foreach ($projects_edit_diff as $groupdn) {
			$groupdn_projectphid_map = $project_map ["by_dn"];
			$phid = $groupdn_projectphid_map [$groupdn];
			$projectdn = str_replace("=groups", "=projects", $groupdn);
			$projectdn = str_replace("cn=", "uid=", $projectdn);
			$project = $ldap_projects [$projectdn];
			$data = array("objectIdentifier" => $phid, "transactions" => array());
			array_push($data["transactions"], array("type" => "name", "value" => $project["displayname"][0]));
			array_push($data["transactions"], array("type" => "description", "value" => $project["displayname"][0]));
// 			array_push($data["transactions"], array("type" => "view", "value" => "PHID-PLCY-obtxrrbinx444phuhjle"));
// 			array_push($data["transactions"], array("type" => "edit", "value" => "admin"));
...
// 			array_push($data["transactions"], array("type" => "custom.bitgear-rs:ldap:dn", "value" => $project["uid"][0]));
	...
	if (DEBUG && ! empty ( $projects_diff ['-'] )) {
		...
		foreach ( $projects_diff ['-'] as $projectphid => $groupdn ) {
			$project = id ( new PhabricatorProjectQuery () )->setViewer ( $phab_admin )->withPHIDs ( array (
					$projectphid 
			) )->executeOne ();
			$projectname = $project->getName ();
...
function update_phab_project_members($project_map, $user_map, $phab_projects, $ldap_groups, $ld, $phab) {
	...
	foreach ( $projectphid_groupdn_map as $projectphid => $groupdn ) {
		...
		foreach ( $group_members as $userdn ) {
			...
			$userdn = "uid=" . $userdn . ",ou=users,dc=bitgear,dc=rs";
